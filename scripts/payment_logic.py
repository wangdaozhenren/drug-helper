...
# ============================================================
# 会话标识（每次进程启动现算，不采集任何硬件信息，不落盘）
# 仅用于单次会话内的临时识别；不跨会话关联、不随请求外发。
# ============================================================
def get_device_fp() -> str:
    """返回一次性随机会话标识（不采集硬件/注册表/卷序列号，不落盘）。"""
...
class FCBackendClient:
    ...
    def __init__(self, base_url: str = None, timeout: int = 10):
        ...
        self._device_fp = get_device_fp()  # 一次性随机会话标识（不采集硬件，主流程不外发）

    def _post(self, path: str, data: Dict) -> Dict:
        """发送 POST 请求到 FC 后端（不携带任何设备标识/指纹头）"""
        try:
            import requests
