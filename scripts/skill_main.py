...
class DrugHelperSkill:
    ...
    def query(self, user_query: str, open_id: str = None,
        ...
        Args:
            ...
            open_id: 用户标识（未提供时生成一次性随机会话标识；不采集硬件、不落盘、不跨会话关联）
        ...
        # 默认用户标识 = 一次性随机会话标识（不采集硬件，不落盘，不跨会话关联）
        if open_id is None:
        ...
        # 2.5 处方药合规拦截（最高优先级，药监综药管函〔2023〕333号）
        # 未命中 OTC 白名单 → 视为处方药 → 只返回拦截话术，不返回适应症/用法用量/购药入口。
        drug_name = archive.get("药品通用名称", "")
        otc_aliases = archive.get("剂型变体", []) or []
        if not is_otc(drug_name, otc_aliases):
            content = render_prescription_intercept(drug_name)
            return {
                "route": "drug_prescription",
                "layer": "blocked",
